...
### Changed
- `modules/query-monitor-lite/class-monitor.php`: 遅いクエリの視認性とコピー機能を強化
  - 遅いクエリ行の SQL テキストと時間をオレンジ色＋太字で強調表示
  - 各遅いクエリ行にコピーボタン（📋）を追加。クリックでクリップボードにコピー
  - コピー成功時は ✓ に変化しアニメーションでフィードバック

### Added
- `modules/query-monitor-lite/`: クエリモニターモジュールを新設
  - ローカル・ステージング環境限定で画面下部に固定バーを表示
  - クエリ数・合計時間・遅いクエリ（デフォルト閾値 50ms）を集計
  - クリックで全クエリ一覧・遅いクエリ詳細を展開（SAVEQUERIES=true 時）
  - `qml/enabled` フィルターで表示環境を上書き可能
- `modules/env-badge/`: 環境バッジ表示モジュールを新設
  - 管理バーにローカル（緑）・ステージング（オレンジ）バッジを表示
  - 本番環境では非表示。ホスト名の部分一致で自動判定
  - `env_badge/detect` / `env_badge/local_hosts` / `env_badge/staging_hosts` フィルターでカスタマイズ可能
